    ...
    if request.method == "POST":
        ...
        return render(
            ...
            "rooms.html",
            {
                "r": r,
                "message": "none",
                "in_d": request.POST.get("check_in"),
                "out_d": request.POST.get("check_out"),
            },
        )
    else:
...
def Book(request, room, in_d, out_d, details):
    ...
    return HttpResponse("Odanız başarıyla rezerve edildi.")


def list_days(a, b):
    o = []
    d = date(int(a.split("-")[0]), int(a.split("-")[1]), int(a.split("-")[2])) - date(
        int(b.split("-")[0]), int(b.split("-")[1]), int(b.split("-")[2])
    )
    d = d.days - (2 * d.days)
